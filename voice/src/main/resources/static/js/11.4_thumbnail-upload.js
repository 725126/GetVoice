...
document.getElementById("submitBtn").addEventListener("click", function(event) {
    ...
    if (!file && !saveFileName) {
     ...
     if (file) {
            ...
            console.log("업로드 파일 용량(썸네일 이미지): ", fileSizeInMB);
            if (fileSizeInMB > 3) {
    ...
    if(!saveFileName || file!=null){
        ...
        if (!allowedExtensions.exec(fileName)) {
            alert('올바른 파일 형식이 아닙니다. jpg 또는 png 파일을 업로드해주세요.');
            return;
        }
        ...
        xhr.open("POST", "/popup/voiceport/vp-thumbnail-endpoint", true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    console.log("(썸네일) 요청이 성공적으로 처리됨");
                    // 팝업 창 닫기
                    if(id != null){
                        alert('썸네일 파일이 수정되었습니다.');
                    }
                    window.close();
                    window.opener.location.reload();
//                    // 썸네일이 설정됨 > 부모페이지의 버튼 텍스트 변경
//                    opener.window.document.querySelector('.setting-button').textContent = '<i class="fas fa-upload"></i> 썸네일 변경';
                } else {
                    // 요청이 실패한 경우 에러를 출력하고 확인
                    console.error("(썸네일) 요청 실패: " + xhr.status);
                }
            }
        };
        xhr.send(formData);
