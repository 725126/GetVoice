...
function handleButtonClick() {
    var button = document.querySelector('.thumbnail.upload-button');
    var buttonText = button.textContent || button.innerText; // 버튼의 텍스트를 가져옴 (크로스 브라우징을 위해 textContent와 innerText 모두 사용)

   if (buttonText.trim() === '썸네일 업로드') {
        // 버튼의 텍스트가 "썸네일 업로드"인 경우
        showpopup_upload1();
    }
}

// "썸네일 변경" 또는 "썸네일 업로드" 버튼 클릭 시 handleButtonClick() 함수 호출
document.querySelector('.thumbnail.upload-button').addEventListener('click', handleButtonClick);


// 수정 요청 함수
function sendThumbnailModifyRequest(id) {
    // AJAX 요청을 보낼 URL 설정
    var url = "/popup/voiceport/vp-thumbnail-modify-endpoint?id=" + id;
        ...
        success: function (response) {
            ...
            console.log("게시물 데이터를 성공적으로 받았습니다:", response);
             if (response) {
                ...
                var popupUrl = "/popup/voiceport/11.4_thumbnail-upload-11.4" +
